...
-------------------------
  ...
  // Dial 单入参 DialReq、单返回 DialResp(成功/失败都返回,Err 内含):
  //   - DialReq.Ctx    拨号/外层 TLS 握手/CONNECT 都跟随其取消与截止时间;nil = context.Background()。
  //   - DialReq.Target 目标 host:port。
  //   - resp.Conn      成功时即到目标的隧道,在其上跑端到端 TLS / 任意字节流;失败为 nil。
  //   - resp.Status    网关 CONNECT 响应码:200 成功;403/407/... = 网关拒绝(resp.Err 非 nil);
  //                    0 = 没拿到响应(拨号/外层 TLS 握手/读响应失败)。按 Status 分类,无需解析 Err 文案。
  //   - resp.Err       失败原因(成功为 nil)。
  // 接 http.Transport.DialContext / gRPC dialer 等只认 (net.Conn, error) 的调用栈:包一层
  //   func(ctx,_,addr)(net.Conn,error){ r:=cfg.Dial(DialReq{Ctx:ctx,Target:addr}); return r.Conn,r.Err }。
  // 注:CONNECT 不再支持注入自定义头——网关侧无 API 读取它们(审计身份取 Basic 用户名)。
...
* hgmHttpsProxyTest    —— 集成测试包,真 TCP 监听 127.0.0.1:0:
  ...
  - features_test.go Dial 经 DialReq.Ctx 取消即返回;隧道空闲超时(到点关·有流量不误杀);
                     DialUpstream 下一跳改投。
